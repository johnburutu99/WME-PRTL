...
import {
  Calendar as CalendarIcon, FileCheck, DollarSign, TrendingUp,
...
function getStatusBadgeColor(status: BookingStatus): string {
  switch (status) {
    ...
    case 'PENDING_REVIEW': return 'bg-slate-500/15 text-slate-400 border-slate-500/30';
    case 'NDA_PENDING':
...
function sumLedger(ledgers: LedgerEntry[], field: keyof LedgerEntry): number {
...
export default function TalentPortal() {
  ...
  const fetchPortalData = useCallback(async () => {
    setFetchError(null);
    if (bookings.length === 0 || ledgers.length === 0) setLoading(true);
    try {
      const [bookingData, ledgerData] = await Promise.all([
        api.get<Booking[]>('/bookings'),
        api.get<LedgerEntry[]>('/bookings/ledger'),
      ]);
  ...
  const handleOfferResponse = async (bookingId: string, action: 'CONFIRMED' | 'OFFER_REJECTED') => {
    ...
    try {
      await api.patch(`/bookings/${bookingId}/respond`, { status: action });
  ...
  // ── Computed ledger KPIs from live data
      ...
      <main className="flex-1 p-6 md:p-10 overflow-y-auto">
        {fetchError && (
          <div role="alert" className="mb-6 bg-red-500/10 border border-red-500/20 text-red-400 p-4 rounded-xl flex items-center justify-between text-sm">
            ...
            <button onClick={fetchPortalData} className="flex items-center gap-1 text-xs underline">
              ...
              <div className="max-w-5xl space-y-8">
                <div>
                  ...
                  <p className="text-slate-400 mt-1.5 text-sm">
                    Your performance agenda. Status automatically advances from Hold to Confirmed upon deposit clearance.
                  </p>
                </div>
                  ...
                  <div className="flex flex-wrap justify-between items-center gap-4">
                    ...
                    <div className="flex gap-4 text-xs font-semibold">
                      ...
                      <span className="flex items-center gap-1.5 text-amber-400">
                        <span className="w-2.5 h-2.5 bg-amber-500 rounded-full" aria-hidden="true" />
                        Negotiation Holds
                      </span>
                    </div>
                  </div>
                  ...
                  {bookings.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {bookings.map((b) => (
                        <div
                          key={b.id}
                          className={`border p-5 rounded-xl flex flex-col justify-between transition-all hover:-translate-y-0.5 ${
                            b.status === 'CONFIRMED'
                        ...
                        >
                          <div>
                            <div className="flex justify-between items-start mb-3">
                              ...
                              <span className={`border font-bold text-[10px] px-2 py-0.5 rounded-full uppercase tracking-wider ${getStatusBadgeColor(b.status)}`}>
                                {b.status}
                              </span>
                            </div>
            ...
            {activeTab === 'offer-desk' && (
              <div className="max-w-4xl space-y-8">
                <div>
                  ...
                  <p className="text-slate-400 mt-1.5 text-sm">
                    Agent-vetted deal memos. Approve or decline incoming bookings below.
                  </p>
                </div>

                {pendingOffers.length === 0 ? (
                  <div className="bg-slate-900 border border-slate-800 rounded-2xl p-10 text-center space-y-3">
                    <CheckCircle className="text-emerald-500 mx-auto" size={40} aria-hidden="true" />
                    <h2 className="text-lg font-bold">No Pending Deals</h2>
                    <p className="text-xs text-slate-400 max-w-md mx-auto">
                      All vetted invitations have been cleared. Check your itinerary for confirmed gigs.
                    </p>
                  </div>
                ) : (
                        ...
                        <div className="space-y-4 flex-1">
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 bg-slate-950 p-4 rounded-xl border border-slate-800 text-xs">
                            <div>
                              ...
                              <div className="font-semibold text-slate-200 mt-0.5">{new Date(offer.eventDate).toLocaleDateString()}</div>
                            </div>
                            <div>
                              <div className="text-slate-500">Venue Capacity</div>
                              <div className="font-semibold text-slate-200 mt-0.5">{Number(offer.venueCapacity).toLocaleString()} seats</div>
                            </div>
                            <div>
                              <div className="text-slate-500">Guaranteed Budget</div>
                              <div className="font-semibold text-amber-400 mt-0.5">${Number(offer.guaranteedBudget).toLocaleString()}</div>
                            </div>
                            <div>
            ...
            {activeTab === 'ledger' && (
              <div className="max-w-5xl space-y-8">
                <div>
                  ...
                  <p className="text-slate-400 mt-1.5 text-sm">
                    Revenue statement of agency commissions, tax withholding, and net payout distributions.
                  </p>
                </div>

                {/* Live KPI cards — computed from actual ledger data */}
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-xl space-y-2">
                    ...
                    <div className="text-[10px] text-amber-500 flex items-center gap-1 font-semibold">
                      ...
                      {ledgers.length > 0 ? 'Awaiting Wire Settlement' : 'No cleared payouts yet'}
                    </div>
                  </div>
                </div>
                        ...
                        {ledgers.length === 0 ? (
                          <tr>
                            <td colSpan={6} className="py-8 px-6 text-center text-xs text-slate-500">
                              No cleared payouts in this trust account yet.
                            </td>
                          </tr>
                        ) : (
                          ledgers.map((l) => (
                            <tr key={l.id} className="hover:bg-slate-800/30 transition-all">
                              <td className="py-4 px-6">
                                ...
                                <div className="text-[10px] text-slate-500 mt-0.5">
                                  {l.booking?.eventDate ? new Date(l.booking.eventDate).toLocaleDateString() : '—'} &bull; {l.booking?.venueName}
                                </div>
                              </td>
