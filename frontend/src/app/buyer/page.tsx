...
import type { Booking, BookingFormSchema, JsonSchemaField, LedgerEntry, Talent } from '@/types/portal';
...
export default function BuyerPortal() {
  ...
  const loadAllData = useCallback(async () => {
    setFetchError(null);
    if (bookings.length === 0 || !schemaFields) setLoading(true);
    try {
      const [schema, bookingData, talentData] = await Promise.all([
        api.get<BookingFormSchema>('/bookings/schema'),
        api.get<Booking[]>('/bookings'),
        api.get<Talent[]>('/bookings/talents'),
      ]);
      ...
      // Derive contracts from bookings that have them
      ...
      const msg = err instanceof ApiError ? err.message : 'Failed to load portal data. Please refresh.';
      setFetchError(msg);
    } finally {
      setLoading(false);
    }
  ...
  const onSubmitIntake = async (values: IntakeFormValues) => {
    ...
    try {
      ...
      setFormSuccess('Booking submitted successfully. Your agent will review it shortly.');
      reset();
      const updated = await api.get<Booking[]>('/bookings');
  ...
  const renderDynamicField = (key: string, prop: JsonSchemaField) => {
    ...
    if (key === 'talentId') {
      return (
        <div key={key}>
          ...
          <select
            id={key}
            {...register('talentId', { required: isRequired })}
    ...
    if (key === 'usageRights') {
      return (
        <div key={key} className="col-span-full">
          ...
          <textarea
            id={key}
            {...register('usageRights', { required: isRequired })}
            rows={3}
            className="w-full bg-slate-950 border border-slate-800 text-slate-100 px-4 py-2.5 rounded-xl text-sm focus:ring-2 focus:ring-amber-500/50"
    ...
    return (
      <div key={key}>
        ...
        />
        {errors[fieldKey] && (
          <p className="text-red-400 text-xs mt-1">{prop.title || key} is invalid or required</p>
        )}
      </div>
    );
  };
      ...
      <aside className={`fixed md:relative inset-y-0 left-0 w-80 bg-slate-900 border-r border-slate-800 p-6 flex flex-col justify-between shrink-0 z-50 md:z-auto transition-transform duration-300 ease-in-out ${
        ...
        <div className="border-t border-slate-800 pt-6">
          <div className="flex items-center gap-3 mb-4">
            <div className="w-10 h-10 bg-slate-800 border border-slate-700 rounded-full flex items-center justify-center font-bold text-amber-500" aria-hidden="true">
      ...
      <main className="flex-1 p-6 md:p-10 overflow-y-auto">
        {fetchError && (
          <div role="alert" className="mb-6 bg-red-500/10 border border-red-500/20 text-red-400 p-4 rounded-xl flex items-center justify-between text-sm">
            ...
            <button onClick={loadAllData} className="flex items-center gap-1 text-xs underline">
              ...
              <div className="max-w-4xl space-y-8">
                <div>
                  ...
                  <p className="text-slate-400 mt-1.5 text-sm">
                    Submit event applications for agent review. Budgets and capacities are run through agency vetting workflows.
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                  ...
                  <div className="lg:col-span-7 bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-xl space-y-6">
                    <div>
                      <h2 className="text-lg font-bold flex items-center gap-2">
                        <FileText className="text-amber-500" size={18} aria-hidden="true" />
                        Booking Intake Form
                      </h2>
                      <p className="text-xs text-slate-500 mt-1">
                        Fields are driven by JSON schema from the agency API.
                      </p>
                    </div>
                  ...
                  <div className="lg:col-span-5 bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-xl space-y-6">
                    <div>
                      <h2 className="text-lg font-bold flex items-center gap-2">
                        <Award className="text-amber-500" size={18} aria-hidden="true" />
                        Active Deal Tracker
                      </h2>
                            ...
                            <div className="flex justify-between items-start">
                              <div>
                                ...
                                <div className="text-xs text-slate-500 mt-0.5">
                                  {b.venueName} &bull; {new Date(b.eventDate).toLocaleDateString()}
                                </div>
                              </div>
            ...
            {activeTab === 'contracts' && (
              <div className="max-w-4xl space-y-8">
                <div>
                  ...
                  <p className="text-slate-400 mt-1.5 text-sm">
                    Access and execute NDAs, riders, and long-form performance contracts inside your authenticated session.
                  </p>
                </div>

                <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-6">
                  <div className="bg-amber-500/5 border border-amber-500/10 p-4 rounded-xl flex gap-3">
                    <ShieldAlert className="text-amber-500 shrink-0 mt-0.5" size={20} aria-hidden="true" />
                    <div>
                      <div className="text-sm font-semibold text-slate-200">Non-Transmittance Policy (NDA Guard)</div>
                      <div className="text-xs text-slate-400 mt-1">
                        WME contracts are never distributed via email. All signatures must be completed inside this portal.
                      </div>
                    </div>
                  </div>
                      ...
                      contracts.map((c) => (
                        <div key={c.id} className="border border-slate-800 rounded-xl p-4 flex justify-between items-center bg-slate-950">
                          <div className="flex items-center gap-3">
                            <FileText className={c.isSigned ? 'text-emerald-500' : 'text-blue-500'} size={24} aria-hidden="true" />
                            <div>
                              <div className="font-semibold text-sm">
                                {c.contractType === 'NDA' ? 'Non-Disclosure Agreement' : 'Long Form Performance Contract'}
                              </div>
                          ...
                          {c.isSigned ? (
                            <a
                              ...
                              className="border border-slate-800 hover:bg-slate-800 text-slate-300 font-semibold text-xs uppercase tracking-widest px-4 py-2 rounded-lg transition-all"
                            >
                              View Document
                            </a>
                          ) : (
                  ...
                  <div className="border border-dashed border-slate-800 rounded-xl p-6 text-center space-y-3">
                    <Lock className="text-slate-500 mx-auto" size={32} aria-hidden="true" />
                    <div className="text-sm font-semibold text-slate-300">48-Hour Lock-In Policy</div>
                    <p className="text-xs text-slate-500 max-w-md mx-auto">
                      Upon receipt of a contract envelope, terms automatically lock and become binding if uncontested within 48 hours.
                    </p>
                  </div>
                </div>
              </div>
            ...
            {activeTab === 'ledger' && (
              <div className="max-w-4xl space-y-8">
                <div>
                  ...
                  <p className="text-slate-400 mt-1.5 text-sm">
                    WME Agency Escrow &amp; Trust Account. Always verify wire instructions inside this portal — never via email.
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-4">
                    <h2 className="text-lg font-bold flex items-center gap-2">
                      <DollarSign className="text-amber-500" size={20} aria-hidden="true" />
                      Verified Escrow Credentials
                    </h2>
                    <p className="text-xs text-slate-400">
                      Phishing protection: wire instructions are never sent via email. Verify all details here inside your authenticated session only.
                    </p>
                  ...
                  <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl flex flex-col justify-between">
                    <div>
                      ...
                      <p className="text-xs text-slate-400 mt-1">
                        Settle 50% non-refundable deposits via ACH or secure card processing.
                      </p>
                    </div>
                    <div className="bg-slate-950 border border-slate-800 rounded-xl p-4 text-center mt-4">
                      ...
                      <button
                        disabled
                        className="w-full bg-slate-800 border border-slate-700 text-slate-400 font-semibold py-2.5 px-4 rounded-xl text-xs uppercase tracking-wider transition-all cursor-not-allowed"
                      >
